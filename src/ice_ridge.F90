...
!                                                                              !
! Prioritized to do list as of 3/9/18 (mw handing off to nnz):                 !
!                                                                              !
! 1) snow_to_ocean (dumping due to ridging) needs to be passed up to increment !
!    fprec field passed to ocean (currently it is just thrown away). Also need !
!    a new snow_to_ocean diagnostic to record this flux.                       !
! 2) check implementation for conservation of heat and mass (and age tracers?) !
! 3) implement ridging_rate diagnostics: ridging_shear, ridging_conv           !
! 4) implement "do_j" style optimization as in "compress_ice" or               !
...
subroutine ice_ridging(part_sz, mca_ice, mca_snow, mca_pond, &
                        mH_ice, mH_snow, mH_pond, TrReg, G, IG, dt, uc, vc, snow_to_ocn)
