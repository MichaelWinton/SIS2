...
! use icepack_tracers, only: tr_pond
  ...
  logical, parameter :: tr_pond = .true. ! temporary 2/8/19 - mw
...
!=======================================================================
  ...
  subroutine two_stage_solver_snow(nilyr,       nslyr,      &
    ...
    if ((Tsf < c0) .and. .not. gfdl_pond) then
       ...
       ! surface conductive heat flux should be less than 
       ! incoming surface heat flux
       if ((fcondtop - fsurfn < ferrmax) .or. gfdl_pond) then
...
!=======================================================================
  ...
  subroutine two_stage_solver_nosnow(nilyr,       nslyr,      &
    ...
    ! determine if surface is initially cold or melting
    if ((Tsf < Tmlt) .and. .not. gfdl_pond) then
...
!=======================================================================
  ...
  subroutine picard_solver(nilyr,    nslyr,    &
    ...
    logical, intent(in) :: gfdl_bc
    real(kind=dbl_kind), intent(in) :: fsurf0, dfsurf0_dT
    ...
    picard: do nit = 1, nit_max
       ...
       ! surface heat flux
       if (gfdl_bc) then
         fsurfn = fsurf0 + dfsurf0_dT*Tsf
         dfsurfn_dTsf = dfsurf0_dT
       else
